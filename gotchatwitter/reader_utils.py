...
def read_input(inputs, input_column=0, input_delimiter=',', input_start_from=None):
...
def read_header(job, header=None):
    readers = {'timeline': _read_header_timeline(header)}
    return readers[job]


def _read_header_timeline(header):
